...
class CacheManager {
	...
	_cleanAll(method) {
		Object.keys(STR).forEach(async strategy => {
			...
			/* try {
				...
				throw new CacheManagerError(`Uninitialized strategy: ${strategy.private.substring(1)}`, CacheManagerError.codes.WITHOUT_RESETTING);
			} */
		});
	}
   ...
   */
	cleanEntity(entity, method) {
		Object.keys(STR).forEach(async strategy => {
			await this[strategy][method](entity);
			/* try {
				await this[strategy][method](entity);
			} catch(error) {
				throw new CacheManagerError('Error when resetting entity by strategies', CacheManagerError.codes.WITHOUT_RESETTING);
			} */
		});
	}
}
